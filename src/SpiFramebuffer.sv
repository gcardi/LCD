...
module SpiFramebuffer (
 ...
 reg [7:0] echo_byte;
 reg [5:0] index;
 reg selected, accept_packet, committed;
 ...
 always @(posedge sck or posedge reset_parser) begin
   if (reset_parser) begin
     index<=0; committed<=0; selected<=0; accept_packet<=0; echo_byte<=8'hA5;
     staging_addr<=0; staging_mask<=0; staging_pixels<=0;
   end else if (push) begin
     echo_byte<=rx;
     if (index == 39) committed <= selected && accept_packet && rx == 8'h5A &&
         staging_addr < 24'd130560 && staging_addr[3:0] == 0;
