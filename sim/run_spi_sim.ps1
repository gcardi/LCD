...
Push-Location $root
try {
foreach ($top in @('tb_spi_slave', 'tb_spi_diagnostic', 'tb_spi_directions', 'tb_spi_framebuffer', 'tb_font_store', 'tb_text_renderer', 'tb_line_renderer', 'tb_blit_renderer', 'tb_framebuffer_fifo', 'tb_boot_logo', 'tb_logo_command')) {
...
try {
    ...
    Invoke-LoggedProcess -FilePath (Join-Path $OssCadSuite 'bin\iverilog.exe') -Arguments @(
        ...
        (Join-Path $root 'src/BlitRenderer.sv'),
        (Join-Path $root 'src/FramebufferFifo.sv'),
